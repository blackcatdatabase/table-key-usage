 ...
 */
final class KeyUsageAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private KeyUsageRepository $keyUsageRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'key_usage' => $this->keyUsageRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\KeyUsage\ModuleException("Record {$idStr} not found for locking.");
            }
