...
use BlackCat\Database\Packages\KeyUsage\Repository\KeyUsageRepositoryInterface;
...
class KeyUsageRepository implements KeyUsageRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;

    /** @var mixed literal token for upsert keys (array or empty). */
    private mixed $tokenUpsertKeys = [ 'key_id', 'usage_date' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\KeyUsage\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'encrypt_count', 'decrypt_count', 'verify_count', 'last_used_at' ]);
          return count($rows);
        }
    ...
    public function getByKeyIdAndUsageDate(int $keyId, \DateTimeInterface|string $usageDate, bool $asDto = false): array|\BlackCat\Database\Packages\KeyUsage\Dto\KeyUsageDto|null {
        return $this->getByUnique([ 'key_id' => $keyId, 'usage_date' => $usageDate ], $asDto);
    }
    ...
    public function getIdByKeyIdAndUsageDate(int $keyId, \DateTimeInterface|string $usageDate) {
        $row = $this->getByKeyIdAndUsageDate($keyId, $usageDate, false);
