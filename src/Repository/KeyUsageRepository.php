...
namespace BlackCat\Database\Packages\KeyUsage\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\KeyUsage\Definitions;
use BlackCat\Database\Packages\KeyUsage\Criteria;
use BlackCat\Database\Packages\KeyUsage\Dto\KeyUsageDto as Dto;
use BlackCat\Database\Packages\KeyUsage\Mapper\KeyUsageDtoMapper as RowMapper;
...
final class KeyUsageRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\KeyUsage\Definitions::class; }
    ...
    private function doUpsert(array $row, bool $revive): void
    {
        ...
        $updCols = [ 'encrypt_count', 'decrypt_count', 'verify_count', 'last_used_at' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'key_id', 'usage_date' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\KeyUsage\Definitions::table());
            return $bulk->upsertMany($rows, [ 'key_id', 'usage_date' ], [ 'encrypt_count', 'decrypt_count', 'verify_count', 'last_used_at' ]);
        }
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\KeyUsage\Dto\KeyUsageDto|null */
    public function getByKeyIdAndUsageDate(int $keyId, \DateTimeInterface|string $usageDate, bool $asDto = false): array|\BlackCat\Database\Packages\KeyUsage\Dto\KeyUsageDto|null {
        $row = $this->getByUnique([ 'key_id' => $keyId, 'usage_date' => $usageDate ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\KeyUsage\Mapper\KeyUsageDtoMapper::fromRow($row);
    }
    public function existsByKeyIdAndUsageDate(int $keyId, \DateTimeInterface|string $usageDate): bool {
        $where = 't.' . Ident::q($this->db, 'key_id') . ' = :uniq_key_id' . ' AND ' . 't.' . Ident::q($this->db, 'usage_date') . ' = :uniq_usage_date';
        return $this->exists($where, [ 'uniq_key_id' => $keyId, 'uniq_usage_date' => $usageDate ]);
    }
    /** @return int|string|null */
    public function getIdByKeyIdAndUsageDate(int $keyId, \DateTimeInterface|string $usageDate) {
        $row = $this->getByKeyIdAndUsageDate($keyId, $usageDate);
        return $row ? ($row['id'] ?? null) : null;
    }
    /** @return array<string,mixed>|\BlackCat\Database\Packages\KeyUsage\Dto\KeyUsageDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\KeyUsage\Dto\KeyUsageDto|null {
